(() => {
  ...
  (() => {
    ...
    const a = {
      ...
      r = (e) => [a.prefix, e, a.suffix].filter((e) => e && e.length > 0).join("-"),
      i = (e) => e || r(a.precache),
      o = (e) => e || r(a.runtime);
    function c(e, t) {
      const s = new URL(e);
        ...
        ? (
          ...
          ? function () {
              for (var t = arguments.length, s = new Array(t), n = 0; n < t; n++)
                s[n] = arguments[n];
              return e.apply(D(this), s), q(_.get(this));
            }
          : function () {
              for (var t = arguments.length, s = new Array(t), n = 0; n < t; n++)
                s[n] = arguments[n];
              return q(e.apply(D(this), s));
            }
        : function (t) {
            for (
            ...
            return b.set(r, t.sort ? t.sort() : [t]), q(r);
          };
    }
    function E(e) {
      return "function" === typeof e
        ...
        : (e instanceof IDBTransaction &&
          ...
          w(
            e,
            m || (m = [IDBDatabase, IDBObjectStore, IDBIndex, IDBCursor, IDBTransaction])
          )
            ? new Proxy(e, C)
            : e);
    }
    function q(e) {
      if (e instanceof IDBRequest)
        return (function (e) {
          ...
          return (
            t
              .then((t) => {
                t instanceof IDBCursor && _.set(t, e);
              })
              .catch(() => {}),
            x.set(t, e),
            t
      ...
      const t = E(e);
      return t !== e && (R.set(e, t), x.set(t, e)), t;
    }
    const D = (e) => x.get(e);
    ...
    function N(e, t) {
      ...
      const s = t.replace(/FromIndex$/, ""),
        n = t !== s,
        a = T.includes(s);
      if (!(s in (n ? IDBIndex : IDBObjectStore).prototype) || (!a && !U.includes(s)))
        return;
      const r = async function (e) {
        ...
        for (
          ...
          o[c - 1] = arguments[c];
        return (
          n && (r = r.index(o.shift())), (await Promise.all([r[s](...o), a && t.done]))[0]
        );
      };
      return k.set(t, r), r;
    }
    C = ((e) => ({
      ...e,
      get: (t, s, n) => N(t, s) || e.get(t, s, n),
      has: (t, s) => !!N(t, s) || e.has(t, s),
    }))(C);
    s(332);
    const I = "cache-entries",
      K = (e) => {
    ...
    class M {
      ...
      _upgradeDb(e) {
        const t = e.createObjectStore(I, { keyPath: "id" });
      ...
      async setTimestamp(e, t) {
        const s = {
          ...
          n = (await this.getDb()).transaction(I, "readwrite", { durability: "relaxed" });
        await n.store.put(s), await n.done;
      }
      async getTimestamp(e) {
        const t = await this.getDb(),
          s = await t.get(I, this._getId(e));
        return null === s || void 0 === s ? void 0 : s.timestamp;
      }
      async expireEntries(e, t) {
        const s = await this.getDb();
        let n = await s.transaction(I).store.index("timestamp").openCursor(null, "prev");
        const a = [];
        let r = 0;
        for (; n; ) {
          const s = n.value;
        ...
        const i = [];
        for (const o of a) await s.delete(I, o.id), i.push(o.url);
        return i;
      }
      _getId(e) {
              ...
              return (
                n &&
                  i.addEventListener("upgradeneeded", (e) => {
                    n(q(i.result), e.oldVersion, e.newVersion, q(i.transaction), e);
                  }),
                s &&
                  i.addEventListener("blocked", (e) => s(e.oldVersion, e.newVersion, e)),
                o
                  .then((e) => {
    ...
    class A {
      ...
      async expireEntries() {
        ...
        (this._isRunning = !1),
          this._rerunRequested && ((this._rerunRequested = !1), l(this.expireEntries()));
      }
    ...
    function P(e) {
      ...
      return a.searchParams.set("__WB_REVISION__", s), { cacheKey: a.href, url: r.href };
    }
    class S {
      constructor() {
        (this.updatedURLs = []),
    ...
    class B {
      ...
      async cachePut(e, s) {
        ...
        if (!s) throw new t("cache-put-with-no-response", { url: d(r.url) });
        const i = await this._ensureResponseSafeToCache(s);
        if (!i) return !1;
        const { cacheName: o, matchOptions: h } = this._strategy,
          l = await self.caches.open(o),
          ...
          p = u
            ? await (async function (e, t, s, n) {
                ...
                const r = Object.assign(Object.assign({}, n), { ignoreSearch: !0 }),
                  i = await e.keys(t, r);
                for (const o of i) if (a === c(o.url, s)) return e.match(o, n);
      ...
      async getCacheKey(e, t) {
        ...
        if (!this._cacheKeys[s]) {
          ...
          for (const e of this.iterateCallbacks("cacheKeyWillBeUsed"))
            n = W(
              await e({ mode: t, request: n, event: this.event, params: this.params })
            );
          this._cacheKeys[s] = n;
        }
      ...
      *iterateCallbacks(e) {
      ...
      async _ensureResponseSafeToCache(e) {
        ...
        for (const n of this.iterateCallbacks("cacheWillUpdate"))
          if (
            ((t =
              (await n({ request: this.request, response: t, event: this.event })) ||
              void 0),
            (s = !0),
            !t)
          )
    ...
    class j {
      ...
      handleAll(e) {
        ...
        const t = e.event,
          ...
          a = new B(this, { event: t, request: s, params: n }),
    ...
    class F extends j {
      ...
      async _handleInstall(e, s) {
        ...
        if (!(await s.cachePut(e, n.clone())))
          throw new t("bad-precaching-response", { url: e.url, status: n.status });
        return n;
      }
        ...
        return p(e, async () => {
          ...
          for (const [a, r] of this._urlsToCacheKeys) {
            const t = this._cacheKeysToIntegrities.get(r),
              ...
              n = new Request(a, { integrity: t, cache: s, credentials: "same-origin" });
            await Promise.all(
    ...
    class Q {
      constructor(e, t) {
        let s = arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : "GET";
      ...
      constructor(e, t, s) {
        super(
          (t) => {
            ...
            if (n && (s.origin === location.origin || 0 === n.index)) return n.slice(1);
          },
          t,
          s
        );
    ...
    class z {
      ...
      handleRequest(e) {
        ...
        const c = t.method;
        if (
          (!o && this._defaultHandlerMap.has(c) && (o = this._defaultHandlerMap.get(c)),
          !o)
        )
          return void 0;
        let h;
          ...
          h instanceof Promise &&
            ...
            (h = h.catch(async (e) => {
              if (l) {
                0;
                try {
                  return await l.handle({ url: n, request: t, event: s, params: r });
                } catch (a) {
                  a instanceof Error && (e = a);
                }
              }
              if (this._catchHandler)
                return this._catchHandler.handle({ url: n, request: t, event: s });
              throw e;
            })),
          h
        );
    ...
    let X;
    const Y = () => (X || ((X = new z()), X.addFetchListener(), X.addCacheListener()), X);
    function Z(e, s, n) {
      let a;
      if ("string" === typeof e) {
        const t = new URL(e, location.href);
      ...
      } else if (e instanceof RegExp) a = new J(e, s, n);
      else if ("function" === typeof e) a = new Q(e, s, n);
      else {
        if (!(e instanceof Q))
    ...
    self.addEventListener("activate", () => self.clients.claim()),
      (function (e) {
        ...
        { revision: "0a31fa5e3ae845e113b743d64446e264", url: "/index.html" },
        { revision: null, url: "/static/css/main.f855e6bc.css" },
        { revision: null, url: "/static/js/592.7f72a991.chunk.js" },
        { revision: null, url: "/static/js/main.41e30bdf.js" },
        {
          revision: "fcd1a3b55ff5c860f001a0f7b76e9b10",
          url: "/static/media/logo.b7c6e29aba1cb4788ff440f3dbbb5ea0.svg",
        },
      ]),
      (function (e) {
        const t = G();
    ...
    Z((e) => {
      ...
      return (
        "navigate" === t.mode && !s.pathname.startsWith("/_") && !s.pathname.match(ne)
      );
    }, ((ae = "/index.html"), G().createHandlerBoundToURL(ae))),
      Z(
        (e) => {
          let { url: t } = e;
            ...
            new (class {
              constructor() {
                let e =
                  arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : {};
                (this.cachedResponseWillBeUsed = async (e) => {
                  let { event: t, request: s, cacheName: n, cachedResponse: a } = e;
                  if (!a) return null;
              ...
              _getCacheExpiration(e) {
                ...
                return (
                  s || ((s = new A(e, this._config)), this._cacheExpirations.set(e, s)), s
                );
              }
